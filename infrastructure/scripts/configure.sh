...
MONGO_KEYFILE="$PROJECT_ROOT/config/mongo-keyfile"
...
# Genera secretos aleatorios seguros (solo alfanuméricos para no romper URIs/YAML)
gen_secret() {
    local len="${1:-32}"
    local s=""
...
write_docker_secret_files() {
    local secret_dir="$PROJECT_ROOT/config/secrets"
...
configure_public_ip() {
    ...
    echo "     [OK] HTTPS automático con certificado válido"
    echo ""
    echo -e "  ${GREEN}2. ngrok${NC}"
    echo "     [OK] Fácil de usar, bueno para pruebas"
...
configure_ngrok() {
    ...
    if [[ "$has_ngrok" =~ ^[Ss]$ ]]; then
        ...
        print_error "ngrok requiere un authtoken y un dominio HTTPS reservado"
        exit 1
    fi

    read -rp "Dominio HTTPS reservado en ngrok (ej: app.ngrok.app): " NGROK_DOMAIN
...
EOF
    ...
    print_success "Archivo .env generado"
    print_info "Caddy gestionará HTTPS automáticamente al iniciar."
}
...
generate_caddyfile() {
    ...
    # Prefer envsubst when available; fall back to portable sed.
    if command -v envsubst >/dev/null 2>&1; then
        export CADDY_PORT="${CADDY_PORT:-4200}"
        export LOCAL_IP="${SELECTED_IP:-}"
        export HTTP_PORT="${HTTP_PORT:-80}"
        export CADDY_INTERNAL_PORT="${CADDY_INTERNAL_PORT:-8080}"
...
ensure_mongo_keyfile() {
    mkdir -p "$(dirname "$MONGO_KEYFILE")"
    chmod 700 "$(dirname "$MONGO_KEYFILE")"
