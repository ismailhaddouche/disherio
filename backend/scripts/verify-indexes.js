...
const expectedIndexes = {
  ...
  orders: [
    ...
    {
      key: { session_id: 1, request_id: 1 },
      options: { unique: true, partialFilterExpression: { request_id: { $type: 'string' } } },
      description: 'Idempotent order requests per session',
    },
  ...
  sessioncustomers: [
    { key: { session_id: 1 }, options: {}, description: 'Session customer lookup' },
    {
      key: { session_id: 1, customer_name_key: 1 },
      options: { unique: true, partialFilterExpression: { customer_name_key: { $type: 'string' } } },
      description: 'Case-normalized customer name uniqueness per session',
    },
  ],
  itemorders: [
    ...
    { key: { last_activity_source: 1, updatedAt: -1 }, options: {}, description: 'Recent activity by source' },
    { key: { last_activity_user_id: 1, updatedAt: -1 }, options: {}, description: 'Recent activity by user' },
  ],
  payments: [
    { key: { session_id: 1 }, options: { unique: true }, description: 'One payment per session' },
    { key: { restaurant_id: 1, payment_date: -1 }, options: {}, description: 'Restaurant payment history' },
    { key: { payment_date: -1 }, options: {}, description: 'Payment statistics by date' },
  ],
  staff: [
  ...
  totemsessions: [
    { key: { totem_id: 1, totem_state: 1 }, options: {}, description: 'Totem sessions by state' },
    {
      ...
      description: 'At most one active session per totem',
    },
...
async function verifyIndexes() {
  try {
    ...
    for (const [collectionName, expected] of Object.entries(expectedIndexes)) {
      ...
      if (!collectionNames.includes(collectionName)) {
        log(`  [WARN]  Collection does not exist yet`, 'yellow');
        continue;
      }
      ...
      for (const expectedIdx of expected) {
        ...
        const found = userIndexes.some(idx => {
          if (!indexesMatch(idx.key, expectedIdx.key)) return false;
          if (expectedIdx.options.unique && idx.unique !== true) return false;
          if (expectedIdx.options.partialFilterExpression) {
            return JSON.stringify(idx.partialFilterExpression) === JSON.stringify(expectedIdx.options.partialFilterExpression);
          }
          return true;
        });
        ...
        if (found) {
          ...
          totalMissing++;
          log(`  [ERROR] ${keyStr}`, 'red');
          log(`     ${expectedIdx.description}`, 'reset');
          log(`     [WARN]  MISSING - Run init-mongo.js to create`, 'yellow');
        }
      }
      ...
      for (const idx of userIndexes) {
        ...
        if (!isExpected) {
          ...
          log(`  [WARN]  ${keyStr} (unexpected/extra)`, 'yellow');
        }
      }
    }
    ...
    if (totalMissing === 0) {
      ...
      log(`\n  [WARN]  ${totalMissing} index(es) need to be created.`, 'yellow');
  ...
  5. Review and remove unused indexes periodically
    ...
    log('\n[OK] Verification complete!', 'green');
