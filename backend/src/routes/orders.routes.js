...
router.patch('/:orderId',
    ...
    [
        param('orderId').isMongoId(),
        body('status').optional().isIn(['active', 'completed', 'cancelled']).withMessage('Invalid order status'),
        body('paymentStatus').optional().isIn(['unpaid', 'paid', 'split', 'processing']).withMessage('Invalid payment status')
    ],
    validate,
    async (req, res) => {
        try {
