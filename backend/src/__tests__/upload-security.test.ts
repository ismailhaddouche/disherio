...
describe('File Security Utilities', () => {
  ...
  describe('getSecurePath', () => {
    ...
    it('should handle path traversal attempts by sanitizing', () => {
      // Path traversal is sanitized, should not throw but result is safe
      const result = getSecurePath(UPLOADS_DIR, 'dishes', '../etc/passwd');
      // Result must be within dishes directory
      expect(result).toContain(`${path.sep}dishes${path.sep}`);
  ...
  describe('validateFileSecurity', () => {
    const createMockFile = (overrides: Partial<Express.Multer.File> = {}): Express.Multer.File => ({
      ...
      mimetype: 'image/jpeg',
      size: 1024,
      buffer: Buffer.concat([Buffer.from([0xff, 0xd8, 0xff]), Buffer.from('fake-image-data')]),
      destination: '',
      filename: '',
      path: '',
      stream: null as any,
