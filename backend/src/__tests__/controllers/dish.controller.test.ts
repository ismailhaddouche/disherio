...
describe('DishController', () => {
  ...
  describe('PATCH /dishes/:id', () => {
    it('should update dish and return updated data', async () => {
      ...
      (dishService.getDishById as jest.Mock).mockResolvedValue({ _id: 'dish123', restaurant_id: 'rest123' });
      ...
      // Assert
      expect(dishService.updateDish).toHaveBeenCalledWith('dish123', 'rest123', req.body);
      expect(jsonMock).toHaveBeenCalledWith(mockDish);
    });
    ...
    it('should return 404 when dish not found', async () => {
      ...
      (dishService.getDishById as jest.Mock).mockResolvedValue(null);
      ...
      expect(errorArg.message).toBe('DISH_NOT_FOUND');
    });
  });
  ...
  describe('PATCH /dishes/:id/toggle', () => {
    it('should toggle dish status and return updated dish', async () => {
      ...
      req.params = { id: 'dish123' };
      (dishService.getDishById as jest.Mock).mockResolvedValue(mockDish);
  ...
  describe('GET /dishes/categories/:id', () => {
    it('should return category by id', async () => {
      ...
      (dishService.getCategoryByIdForRestaurant as jest.Mock).mockResolvedValue(mockCategory);

      // Act
      await getCategory(req as Request, res as Response, next);

      // Assert
      expect(dishService.getCategoryByIdForRestaurant).toHaveBeenCalledWith('cat123', 'rest123');
    ...
    it('should return 404 when category not found', async () => {
      ...
      (dishService.getCategoryByIdForRestaurant as jest.Mock).mockResolvedValue(null);
      ...
      const errorArg = (next as jest.Mock).mock.calls[0][0];
      expect(errorArg).toBeInstanceOf(AppError);
      expect(errorArg.message).toBe('CATEGORY_NOT_FOUND');
    });
  });
  ...
  describe('PATCH /dishes/categories/:id', () => {
    it('should update category and return updated data', async () => {
      ...
      (dishService.getCategoryByIdForRestaurant as jest.Mock).mockResolvedValue({ _id: 'cat123', restaurant_id: 'rest123' });
