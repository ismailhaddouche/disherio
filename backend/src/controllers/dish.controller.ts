...
export const updateDish = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  const dish = await DishService.updateDish(String(req.params.id), req.user!.restaurantId, req.body);
  if (!dish) {
    throw createError.notFound('DISH_NOT_FOUND');
  }
  res.json(dish);
...
export const updateCategory = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  // Verify ownership BEFORE updating to prevent IDOR
  const existing = await DishService.getCategoryByIdForRestaurant(
    String(req.params.id),
    req.user!.restaurantId
  );
  if (!existing) {
