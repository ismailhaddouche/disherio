...
if [ "$LANG_OPT" = "2" ]; then
    ...
    MSG_DNS_REQUIRED="REQUIRED (your site won't work without these)"
    MSG_DNS_RECOMMENDED="RECOMMENDED (SSL certificate & security)"
    MSG_DNS_CAA_NOTE="The CAA record authorizes Let's Encrypt to issue SSL certificates for your domain."
else
    ...
    MSG_DNS_REQUIRED="OBLIGATORIOS (tu sitio no funcionará sin estos)"
    MSG_DNS_RECOMMENDED="RECOMENDADOS (certificado SSL y seguridad)"
    MSG_DNS_CAA_NOTE="El registro CAA autoriza a Let's Encrypt a emitir certificados SSL para tu dominio."
fi
...
if [ "$IS_PUBLIC_DOMAIN" = "true" ]; then
    ...
    if [ -n "$VPS_PUBLIC_IP" ]; then
        echo -e ""
        echo -e "  ${GREEN}── ${MSG_DNS_REQUIRED} ──${NC}"
        echo -e ""
        echo -e "  ┌──────────┬─────────────────────────┬──────────────────────────────────┐"
        ...
        echo -e "  └──────────┴─────────────────────────┴──────────────────────────────────┘"
        echo -e ""
        echo -e "  ${YELLOW}── ${MSG_DNS_RECOMMENDED} ──${NC}"
        echo -e ""
        echo -e "  ┌──────────┬─────────────────────────┬──────────────────────────────────┐"
        printf  "  │ %-8s │ %-23s │ %-32s │\n" "${MSG_DNS_TYPE}" "${MSG_DNS_HOST}" "${MSG_DNS_VALUE}"
        echo -e "  ├──────────┼─────────────────────────┼──────────────────────────────────┤"
        printf  "  │ ${GREEN}%-8s${NC} │ ${CYAN}%-23s${NC} │ ${CYAN}%-32s${NC} │\n" "CAA" "${CADDY_DOMAIN}" "0 issue \"letsencrypt.org\""
        printf  "  │ ${GREEN}%-8s${NC} │ ${CYAN}%-23s${NC} │ ${CYAN}%-32s${NC} │\n" "CAA" "${CADDY_DOMAIN}" "0 issuewild \"letsencrypt.org\""
        echo -e "  └──────────┴─────────────────────────┴──────────────────────────────────┘"
        echo -e ""
        echo -e "  ${YELLOW}${MSG_DNS_CAA_NOTE}${NC}"
        ...
        echo -e ""
        echo -e "  ${GREEN}── ${MSG_DNS_REQUIRED} ──${NC}"
        echo -e "  ${MSG_DNS_TYPE}: A    | ${MSG_DNS_HOST}: ${CADDY_DOMAIN}       | ${MSG_DNS_VALUE}: <IP>"
        echo -e "  ${MSG_DNS_TYPE}: A    | ${MSG_DNS_HOST}: www.${CADDY_DOMAIN}   | ${MSG_DNS_VALUE}: <IP>"
        echo -e ""
        echo -e "  ${YELLOW}── ${MSG_DNS_RECOMMENDED} ──${NC}"
        echo -e "  ${MSG_DNS_TYPE}: CAA  | ${MSG_DNS_HOST}: ${CADDY_DOMAIN}       | ${MSG_DNS_VALUE}: 0 issue \"letsencrypt.org\""
        echo -e "  ${MSG_DNS_TYPE}: CAA  | ${MSG_DNS_HOST}: ${CADDY_DOMAIN}       | ${MSG_DNS_VALUE}: 0 issuewild \"letsencrypt.org\""
    fi
fi
