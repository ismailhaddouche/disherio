...
describe('TasService', () => {
  ...
  describe('Totems', () => {
    it('should get totems', () => {
      const mockTotems: Totem[] = [
        { _id: '1', totem_name: 'Mesa 1', totem_type: 'STANDARD', totem_qr: 'qr1', restaurant_id: 'rest1' },
        { _id: '2', totem_name: 'Mesa 2', totem_type: 'TEMPORARY', totem_qr: 'qr2', restaurant_id: 'rest1' }
      ];
  ...
  describe('Orders & Items', () => {
    ...
    it('should preserve the batch request id when RxJS retries', () => {
      const items = [{ dishId: 'dish1', quantity: 1 }];
      service.addBatchItems('session1', items).pipe(retry(1)).subscribe();

      const first = httpMock.expectOne(`${environment.apiUrl}/orders/items/batch`);
      ...
      const retried = httpMock.expectOne(`${environment.apiUrl}/orders/items/batch`);
      ...
      retried.flush({ orderId: 'order1', items: [] });
    });
