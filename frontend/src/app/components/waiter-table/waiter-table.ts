...
import { WaiterTableViewModel } from './waiter-table.viewmodel';

@Component({
  selector: 'app-waiter-table',
  standalone: true,
  imports: [CommonModule, FormsModule, LucideAngularModule, TranslateModule],
  providers: [WaiterTableViewModel],
  template: `
    <div class="customer-container animate-fade-in">
      @if (vm.loading()) {
        ...
        <div class="name-selection md-card-elevated">
          ...
          <h2 class="text-headline-medium">{{ vm.error() }}</h2>
          <button class="btn-primary full-width action-button" (click)="vm.backToWaiterPanel()">
        ...
        <header class="md-card table-header">
          <div class="table-header-main">
            <div class="restaurant-info">
              <div class="title-row">
                <button class="icon-btn-md3" (click)="vm.backToWaiterPanel()">
                  <lucide-icon name="arrow-left" [size]="18"></lucide-icon>
                </button>
              ...
              <div class="table-header-meta">
                ...
                <span class="status-chip waiter-chip">{{ 'ROLES.waiter' | translate }}</span>
                @if (vm.session()?.activeOrder) {
                  <span class="status-chip">{{ 'WAITER.STATUS_OCCUPIED' | translate }}</span>
                }
              </div>
            </div>
            <button class="btn-secondary btn-sm" (click)="vm.openCustomItemModal()">
        ...
        <main class="menu-content">
          <section class="welcome-hero md-card">
            ...
            <h1 class="text-headline-large">{{ 'WAITER.PANEL' | translate }}</h1>
            <p class="text-body-medium">{{ 'WAITER.DESC' | translate }}</p>
          </section>
                  ...
                  <div class="item-details">
                    ...
                    <div class="item-footer">
                      ...
                      @if (item.variants?.length > 0 || item.addons?.length > 0) {
                        <span class="text-label-large opacity-60">+ {{ 'CUSTOMER.CUSTOMIZE' | translate }}</span>
                      }
                    </div>
                  </div>
                      ...
                      <section class="config-section">
                        ...
                        <div class="options-row">
                          @for (opt of sec.options; track opt) {
                            <button class="chip" [class.selected]="vm.selectedMenuChoices()[sec.name] === opt" (click)="vm.selectMenuChoice(sec.name, opt)">
                              {{ opt }}
                            </button>
                          }
                        </div>
                    ...
                    <section class="config-section">
                      ...
                      <div class="options-row">
                        @for (v of item.variants; track v.name) {
                          <button class="chip variant-chip" [class.selected]="vm.selectedVariant()?.name === v.name" (click)="vm.selectedVariant.set(v)">
          ...
          @if (vm.showCustomItemModal()) {
            <div class="md-modal-overlay customer-modal-overlay" (click)="vm.showCustomItemModal.set(false)">
              <div class="md-modal-bottom-sheet customer-sheet" (click)="$event.stopPropagation()">
                <div class="sheet-drag-handle"></div>
                <header class="config-header">
                  <div>
                    <span class="text-label-large">{{ 'POS.ADD_CUSTOM_LINE' | translate }}</span>
                    <h2 class="text-headline-small config-title">{{ 'POS.CUSTOM_ITEM' | translate }}</h2>
                  </div>
                </header>

                <div class="config-content">
                  <div class="md-input-field left-aligned-field">
                    <input type="text" class="name-input left-aligned-input" [placeholder]="'POS.CUSTOM_NAME_PH' | translate" [value]="vm.customItemName()" (input)="vm.customItemName.set($any($event.target).value)">
                  </div>
                  <div class="md-input-field left-aligned-field">
                    <input type="number" min="0" step="0.01" class="name-input left-aligned-input" [placeholder]="'POS.PRICE' | translate" [value]="vm.customItemPrice() ?? ''" (input)="vm.customItemPrice.set($any($event.target).value === '' ? null : +$any($event.target).value)">
                  </div>
                </div>
                  ...
                  <div class="md-card history-item">
                    ...
                    <div class="item-meta">
                      ...
                      <span class="item-status" [class]="item.status">
                        {{ item.status === 'pending' ? ('CUSTOMER.RECEIVED' | translate) : item.status === 'preparing' ? ('CUSTOMER.ON_FIRE' | translate) : item.status === 'ready' ? ('CUSTOMER.SERVED' | translate) : ('CUSTOMER.COMPLETED' | translate) }}
                      </span>
                    </div>
                  </div>
                }
...
export class WaiterTableComponent {
  public vm = inject(WaiterTableViewModel);
}
