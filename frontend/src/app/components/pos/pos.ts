          ...
          @if (table.status === 'occupied') {
            ...
            <div class="checkout-actions">
              <button class="btn-edit" (click)="vm.toggleEditMode()">
                ...
                {{ vm.editMode() ? ('POS.DONE' | translate) : ('POS.EDIT' | translate) }}
              </button>
              <button class="btn-split" (click)="vm.openSplitModal()">
                <lucide-icon name="users" [size]="14" class="inline-icon"></lucide-icon> {{ 'POS.SPLIT' | translate }}
              </button>
              <button class="btn-print" (click)="vm.processPayment()">
                <lucide-icon name="credit-card" [size]="14" class="inline-icon"></lucide-icon> {{ 'POS.PAYMENT' | translate | uppercase }} {{ 'POS.TOTAL_SIMPLE' | translate }}
              </button>
            </div>
          </div>
              <div class="ticket-content">
                <section class="comensales-breakdown">
                  <h3>{{ 'POS.BREAKDOWN' | translate }}</h3>
                ...
                <section class="pos-total-section">
                  @if (vm.calculateBilling(table.order.totalAmount); as billing) {
                    <div class="total-row">
                      <span>{{ 'POS.BASE_PRICE' | translate }}</span>
                      <span>{{ billing.basePrice | currency:'EUR' }}</span>
                    </div>
                    <div class="total-row">
                      <span>{{ 'POS.VAT' | translate }} ({{ billing.vatPercentage }}%)</span>
