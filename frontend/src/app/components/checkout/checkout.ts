  ...
  template: `
    <div class="checkout-container">
      <header class="checkout-header">
        <button routerLink="../" class="back-btn">{{ 'CHECKOUT.BACK' | translate }}</button>
        <h1 class="gradient-text">{{ 'CHECKOUT.TITLE' | translate }}</h1>
      </header>

      @if (vm.loading()) {
        <div class="loader">{{ 'CHECKOUT.CALCULATING' | translate }}</div>
      } @else if (!vm.order()) {
        <div class="glass-card error-card">
          <p>{{ 'CHECKOUT.NO_ACCOUNT' | translate }}</p>
          <button routerLink="../" class="btn-primary">{{ 'CHECKOUT.BACK_TO_MENU' | translate }}</button>
        </div>
      } @else {
        ...
        <div class="glass-card breakdown-section">
          @if (vm.paymentMode() === 'total') {
            <div class="total-view">
              <span class="label">{{ 'CHECKOUT.TOTAL_PAY' | translate }}</span>
          ...
          @if (vm.paymentMode() === 'individual') {
            <div class="individual-view">
              <h3>{{ 'CHECKOUT.BREAKDOWN_USER' | translate }}</h3>
          ...
          @if (vm.paymentMode() === 'equitativo') {
            <div class="equitativo-view">
              <span class="label">{{ 'CHECKOUT.EQUITATIVE_PARTS' | translate }} ({{ vm.comensales().length }} {{ 'CHECKOUT.PERS' | translate }})</span>
              <h2 class="amount gradient-text">{{ vm.equitativoAmount() | currency:'EUR' }}</h2>
              <p class="helper-text">{{ 'CHECKOUT.EQUITATIVE_HELPER' | translate }}</p>
            </div>
          }
        </div>

        <button class="btn-primary pay-now-btn" (click)="vm.processPayment()">
          {{ 'CHECKOUT.PROCEED_PAY' | translate }}
        </button>
      }
    </div>
  `,
