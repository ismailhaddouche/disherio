...
export const tasStore: TasStore = {
  ...
  availableTotems: computed(() => {
    // Per-totem: exclude only totems that already have a STARTED session.
    // Multiple tables can be open simultaneously (one session per table).
    const activeTotemIds = new Set(
      _sessions().filter(s => s.totem_state === 'STARTED').map(s => s.totem_id?.toString())
    );
  ...
  addItem(item: ItemOrder) {
    _sessionItems.update(current => current.some(existing => existing._id === item._id)
      ? current
      : [...current, item]);
  },
  ...
  updateSessionState(sessionId: string, newState: TotemSession['totem_state']) {
    ...
    const selected = _selectedSession();
    if (selected?._id === sessionId) {
      if (newState === 'PAID' || newState === 'CANCELLED') {
        ...
        _selectedSession.set({ ...selected, totem_state: newState });
      }
    }
  },
