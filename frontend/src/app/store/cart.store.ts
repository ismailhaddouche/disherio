...
export interface CartStore {
  ...
  removeItem: (dishId: string, variantId?: string, extras?: Extra[], customerId?: string) => void;
  decrementItem: (dishId: string, variantId?: string, extras?: Extra[], customerId?: string) => void;
  clear: () => void;
}
...
// Internal computed value avoids a circular reference.
...
function extrasKey(extras: Extra[] = []): string {
  return extras.map(extra => extra.extraId).sort().join('|');
}
...
function matchesItem(
  ...
  return item.dishId === dishId
    ...
    && extrasKey(item.extras) === extrasKey(extras);
}

function findItemIndex(items: CartItem[], item: Omit<CartItem, 'quantity'>): number {
  return items.findIndex(candidate => matchesItem(
    candidate,
    item.dishId,
    item.variantId,
    item.extras,
...
export const cartStore: CartStore = {
  ...
  addItem(item: Omit<CartItem, 'quantity'>) {
    _items.update((current) => {
      const existingIndex = findItemIndex(current, item);
  ...
  removeItem(dishId: string, variantId?: string, extras: Extra[] = [], customerId?: string) {
    _items.update((current) =>
      current.filter((item) => !matchesItem(item, dishId, variantId, extras, customerId))
    );
  },

  decrementItem(dishId: string, variantId?: string, extras: Extra[] = [], customerId?: string) {
    _items.update((current) => {
      const index = current.findIndex(item => matchesItem(item, dishId, variantId, extras, customerId));
